...
long multiply_row(long ** A, long ** B, size_t num_cols_a, size_t i, size_t j) {
    long C = 0;
    for (size_t k=0; k<num_cols_a; k++) {
                C += A[i][k] * B[k][j];
            }
...
void parallel_matrix_multiply2(long ** A, size_t num_rows_a, size_t num_cols_a,
                              long ** B, size_t num_rows_b, size_t num_cols_b,
							  long ** C) {
    /***********************
     * YOUR CODE GOES HERE *
    ...
    std::vector<std::future<long>> CF(num_rows_a * num_cols_b);
    for (size_t i=0; i<num_rows_a; i++) {
        for (size_t j=0; j<num_cols_b; j++) {
			CF[i+j*num_rows_a] = std::async(std::launch::async, multiply_row, A, B, num_cols_a, i, j);
		}
	}

    for (size_t i=0; i<num_rows_a; i++) {
        for (size_t j=0; j<num_cols_b; j++) {
            C[i][j] = CF[i+j*num_rows_a].get();
        }
    }
}
...
void parallel_matrix_multiply(long ** A, size_t num_rows_a, size_t num_cols_a,
                              long ** B, size_t num_rows_b, size_t num_cols_b,
							  long ** C) {
    const size_t num_workers = 4;//std::thread::hardware_concurrency();
